...
function applySFlag(data) {
  ...
  let removedSpaces = [];
  let splittedData = data.split("\r\n");//splits in terms of given parameter
...
function applyBFlag(data){
    ...
    for(let i=0 ; i<splittedData.length ; i++){
        if(splittedData[i] != ''){
            splittedData[i] = `${count}. ${splittedData[i]}`; //(${expression}). The expressions in the placeholders and the text between the backticks (` `) get passed to a function.
